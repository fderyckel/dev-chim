...
The separate [`retained_data_migration_rehearsal`](priv/retained_data_migration_rehearsal/) runs an additive expand, mixed-version SQL compatibility window, typed tenant-scoped batched backfill, reversible enforcement gate, separate constraint validation, and explicitly irreversible contract only against disposable databases. Its [evidence note](../../docs/phase-0/evidence/retained-data-migration-rehearsal.md) records the bounded platform-owned choreography that generated schema artifacts do not provide.

The checked [maintenance boundary manifest](priv/maintenance/owned-boundaries.json) inventories every current non-atomic action, raw-SQL source group, generated-interface edge adapter, OpenAPI modifier, and retained-data choreography path with ownership and a closure gate. Focused tests reject unowned additions, authorization bypasses, silent skips, and detached controls; the [maintenance evidence](../../docs/phase-0/evidence/ash-test-and-maintenance-ergonomics.md) records seeded, isolated, compile-feedback, and warning-as-error results.
...
The completed [dependency upgrade exercise](../../docs/phase-0/evidence/ash-upgrade-exercise.md) compares the immediately preceding framework patches with the current lock in a disposable copy. It records package changes, compile warnings, security-audit results, migration output, and the same 21-test compatibility result without turning network-dependent freshness checks into the local verification contract. The checked [warning baseline](priv/maintenance/ash-dependency-warnings.json) and [normalizer evidence](../../docs/phase-0/evidence/ash-dependency-warning-baseline.md) force the complete locked dependency compile and reject any added or removed normalized warning group, while application warnings remain fatal.

The neutral [`TrustedRouting`](lib/ash_foundation_lab/trusted_routing.ex) slice resolves authenticated tenant context through a versioned registry into real pooled or dedicated test databases and tenant-qualified infrastructure targets. Requests cannot select placement, spawned tasks require explicit propagation, and jobs plus ten non-HTTP interface classes re-resolve current placement from an allowlisted envelope before re-entering Ash authorization. Named, capability-gated movement keeps the source authoritative during copy, quiesces work, compares code-owned tenant snapshots across both real databases, and permits only reconciled versioned cutover or pre-cutover rollback. Every missing, stale, unavailable, mismatched, or quiesced route fails before work begins. The [routing evidence](../../docs/phase-0/evidence/trusted-routing.md) records the limits of this disposable control-plane stand-in and direct-copy rehearsal.
...
The neutral [`ResourceDescriptor`](lib/ash_foundation_lab/resource_descriptor.ex) and [`GovernedMetadata`](lib/ash_foundation_lab/governed_metadata.ex) slice derives one checked, stable-reference descriptor from the allowlisted Ash model and validates tenant view/report definitions without copying type, policy, tenancy, or persistence authority. Approved report filters become sanitized Ash filter input and execution re-enters the existing policy boundary with the real actor and tenant. Private, stale, cross-tenant, arbitrary-SQL/code, authority, and unapproved-action references fail closed. The [scenario 15 evidence](../../docs/phase-0/evidence/resource-authoring-and-governed-metadata.md) records rename and patch-upgrade ergonomics, cost, and limits; no production metadata engine or renderer is introduced.
