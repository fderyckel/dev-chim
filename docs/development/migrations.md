...
- ADR 0018 T1-C adds immutable fact-operation and consumer-basis tables. The reviewed migration separates unrelated generated lifecycle drift, creates the self-referencing consumer foreign key only after its `(id, tenant_id)` destination index, replaces the existing fact guard in place, drops the self-reference before its index on rollback, and restores the T1-A fact guard. Empty synthetic rollback/reapply passes; retained fact operations or consumer bases make rollback fail closed.
