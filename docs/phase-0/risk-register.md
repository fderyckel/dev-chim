...
| R-09 | Trusted placement routing sends a tenant to the wrong database, queue, or storage namespace | Critical | Authenticated versioned registry, placement membership constraints, no default fallback, and [cross-placement, non-HTTP, versioned-movement, reconciliation, and rollback tests](evidence/trusted-routing.md); durable production integration remains gated | ADR 0003 and threat-model review | Security architecture |
...
| R-15 | Generated interface behaviour or documentation diverges from the public contract | High | Thin edge enforcement, supported OpenAPI modification, checked-in OpenAPI and generated-TypeScript drift detection, compile-time invalid-call assertions, and transport-level negative tests | ADR 0014 review | Platform and web engineering |
| R-16 | Retried or conflicting action submissions duplicate state/events or suppress a different request | High | Transactional tenant-scoped claims bound to action, actor, aggregate, and canonical request; exact replay, stable conflict, two-connection serialization, and rollback tests | ADR 0005 and ADR 0014 review | Platform engineering and security architecture |
| R-17 | A schema change loses retained tenant data, blocks peak work, or contracts while an old version still writes | Critical | [Nullable expand, mixed-version, tenant-batched backfill, lock-budget, validation, rollback-window, and destructive-contract gate rehearsal](evidence/retained-data-migration-rehearsal.md) | ADR 0002 review and production migration runbook | Platform engineering and operations |
| R-18 | A metadata convenience layer becomes a second schema, workflow, or authorization engine | Critical | Code-defined Ash authority, one-way descriptor derivation, allowlisted metadata, promotion rule, and authoring-scenario negative tests | ADR 0019 and threat-model review | Platform engineering and security architecture |
