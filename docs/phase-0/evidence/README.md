# Phase 0 evidence

Evidence records what was actually executed or reviewed. It does not copy large generated logs into Git and does not contain secrets or real data.

Each evidence note records:

- date and environment;
- source revision;
- exact command;
- relevant versions;
- result and failures;
- links to tests or artifacts; and
- limits of what the result proves.

- [Ash pressure-test](ash-pressure-test.md)
- [Ash dependency upgrade exercise](ash-upgrade-exercise.md)
- [AshJsonApi non-patch upgrade exercise](ash-nonpatch-upgrade-exercise.md)
- [Ash security-patch reviews](ash-security-patch.md)
- [Ash dependency warning baseline](ash-dependency-warning-baseline.md)
- [Ash test and maintenance ergonomics](ash-test-and-maintenance-ergonomics.md)
- [Ash bounded-condition disposition](ash-bounded-condition-disposition.md)
- [Resource authoring and governed metadata](resource-authoring-and-governed-metadata.md)
- [Retained-data expand-and-contract migration rehearsal](retained-data-migration-rehearsal.md)
- [Retained-data migration annual-envelope measurement](retained-data-migration-measurement.md)
- [Trusted tenant-placement routing, movement, and non-HTTP scoping](trusted-routing.md)
- [Threat-model review](threat-model-review.md)
- [Quality-target approval](quality-targets-approval.md)
- [Quality-target recommendation](quality-target-recommendation.md)
- [Five-school capacity and PostgreSQL recovery measurement](capacity-and-recovery-measurement.md)
- [Tenant-fairness and backpressure measurement](tenant-fairness-backpressure-measurement.md)
- [Pre-checkout tenant-admission measurement](precheckout-admission-measurement.md)
- [Withdrawn AWS PostgreSQL topology example](managed-postgresql-topology.md)
- [Full planning-horizon restore measurement](full-horizon-restore-measurement.md)
- [Clean-checkout rehearsal](clean-checkout-rehearsal.md)
- [Tenant-placement capacity](tenant-placement-capacity.md)
- [PostgreSQL availability and synchronized burst](postgresql-availability-and-burst.md)
- [Module lifecycle](module-lifecycle.md)
