...
| Ash Foundation Lab | Scenarios 1-14 now have evidence; role composition has cycle-safe administration plus field/relationship policy proof, and the generated interface has versioned routes, stable core errors, bounded keyset pagination, transactional action idempotency, checked-in OpenAPI/types drift detection, and a tested thin TypeScript client. Remaining error categories, placement movement, and the wider adoption scorecard remain incomplete | [Ash pressure-test and scorecard](evidence/ash-pressure-test.md), [routing evidence](evidence/trusted-routing.md), and [module-lifecycle evidence](evidence/module-lifecycle.md) |
