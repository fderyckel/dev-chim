...
This repository is establishing the governed foundation for a modular school platform. Phase 0 completed its architecture, security-baseline, pressure-test, and accountable decision package on 2026-09-16. Ash is conditionally accepted as the default production-core framework, with eight binding production gates. Phase 1 slices 1A through 1H-A, ADR 0018 T1-A temporal physical-model qualification and T1-B governed revision boundary, and the local synthetic UI-0 browser experience are implemented. Slice 1H-A adds neutral independent module gates and safe initial activation; it does not add a school business module.
